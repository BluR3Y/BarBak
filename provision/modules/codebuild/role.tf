resource "aws_iam_role" "main" {
  name = "${var.project_name}-${var.project_environment}-build-role"
...
resource "aws_iam_policy" "main" {
  name = "${var.project_name}-${var.project_environment}-build-policy"
  description = "Policy to allow codebuild to execute"
  policy = data.aws_iam_policy_document.codebuild_barbak_app.json
}
...
data "aws_iam_policy_document" "codebuild_barbak_app" {
  statement {
    effect = "Allow"
